    ...
    if ($_SERVER['REQUEST_METHOD'] === 'POST') {
        ...
        if(isset($_FILES['imagen'])){
            ...
            if (empty($errors)==true) {
              ...
              $res = $database->addEvento($titulo, $organizador, $fecha, $lugar, $descripcion, $enlace, $etiquetas, $file_name);
              if($res['resultado']){
                  $variablesParaTwig['resultado'] = "insertado";
                  $idNuevoEvento = $res['id'];
              }
              else{
        ...
        if(isset($_FILES['imagenes'])){
            ...
            for($i = 0; $i < $total_ficheros; $i++){
              ...
              if (empty($errors2)==true) {
                ...
                $database->addImagenEvento($idNuevoEvento, $file_name2);
      ...
      echo $twig->render('add_evento.html', $variablesParaTwig);
    ?>